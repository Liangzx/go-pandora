...
func main() {
	...
	kt, err := keytab.Load("/etc/security/keytabs/http-liangzxgg.keytab")
	if err != nil {
		log.Fatal(err)
	}
	...
	mux.Handle("/hello", spnego.SPNEGOKRB5Authenticate(http.HandlerFunc(hello), kt, service.Logger(l), service.SessionManager(NewSessionMgr("gokrb5"))))
	mux.Handle("/world", spnego.SPNEGOKRB5Authenticate(http.HandlerFunc(world), kt, service.Logger(l), service.SessionManager(NewSessionMgr("gokrb5"))))
